...
class VerifyRuntime:
    def __init__(self, config: VerifyConfig, profiler: Optional[VerifyProfiler] = None):
        ...
        # CPU state for chain verification: each layer's chain reads its
        # input from here and stores its output for the next layer.
        self._cpu_state: Dict[str, torch.Tensor] = {}
        self._cpu_state_events: Dict[str, threading.Event] = {}
        self._cpu_state_d2h: Dict[str, torch.cuda.Event] = {}
        self._cpu_state_lock = threading.Lock()
    ...
    # ── CPU state management for chain verification ─────────────────────

    def cpu_state_set_d2h(
        self, key: str, cpu_tensor: torch.Tensor, cuda_event: Optional[torch.cuda.Event] = None,
    ) -> None:
        """Store a D2H'd tensor.  The consumer will synchronize *cuda_event*."""
        with self._cpu_state_lock:
            self._cpu_state[key] = cpu_tensor
            if cuda_event is not None:
                self._cpu_state_d2h[key] = cuda_event
            if key not in self._cpu_state_events:
                self._cpu_state_events[key] = threading.Event()
            self._cpu_state_events[key].set()

    def cpu_state_set(self, key: str, tensor: torch.Tensor) -> None:
        """Store a tensor computed by a CPU chain (no D2H event)."""
        with self._cpu_state_lock:
            self._cpu_state[key] = tensor
            self._cpu_state_d2h.pop(key, None)
            if key not in self._cpu_state_events:
                self._cpu_state_events[key] = threading.Event()
            self._cpu_state_events[key].set()

    def cpu_state_get(self, key: str, timeout: float = 120.0) -> torch.Tensor:
        """Wait for and return CPU state.  Synchronizes any pending D2H."""
        with self._cpu_state_lock:
            if key not in self._cpu_state_events:
                self._cpu_state_events[key] = threading.Event()
            evt = self._cpu_state_events[key]
        if not evt.wait(timeout):
            raise TimeoutError(f"Timeout waiting for CPU state: {key}")
        with self._cpu_state_lock:
            cuda_evt = self._cpu_state_d2h.pop(key, None)
        if cuda_evt is not None:
            cuda_evt.synchronize()
        return self._cpu_state[key]

    def cpu_state_clear(self) -> None:
        """Clear all CPU state (call between forward passes)."""
        with self._cpu_state_lock:
            self._cpu_state.clear()
            self._cpu_state_d2h.clear()
            for evt in self._cpu_state_events.values():
                evt.clear()
            self._cpu_state_events.clear()
    ...
    def flush(self) -> None:
        ...
        # Reset CPU chain state so the next forward pass starts fresh.
        self.cpu_state_clear()
