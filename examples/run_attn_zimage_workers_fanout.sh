#!/usr/bin/env bash
# Launch N multi-machine attn-zimage worker processes on THIS machine, one per
# port (PORT_BASE .. PORT_BASE+N-1). Just delegates to run_attn_zimage_worker.sh
# per worker. Pair with run_attn_zimage_coordinator_fanout.sh on the other side.
#
# Usage:
#   examples/run_attn_zimage_workers_fanout.sh                         # 5 workers
#   N=5 DEVICE=cuda examples/run_attn_zimage_workers_fanout.sh         # round-robin all GPUs
#   FAULT=drop_qk_norm examples/run_attn_zimage_workers_fanout.sh
#   N=5 DETACH=1 examples/run_attn_zimage_workers_fanout.sh
#
# Env knobs:
#   N          number of worker processes                (default: 5)
#   PORT_BASE  first port; uses PORT_BASE..PORT_BASE+N-1 (default: 9102)
...
#   LOGDIR     dir for per-worker logs + pidfile         (default: workers_attn_zimage_<ts>)
...
LOGDIR="${LOGDIR:-workers_attn_zimage_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "$LOGDIR"
...
echo "[workers] launching ${N} attn-zimage worker(s)   ports ${PORT_BASE}..$((PORT_BASE + N - 1))   fault: ${FAULT}   pipeline: ${PIPELINE}   loop: ${LOOP}"
...
for ((i = 0; i < N; i++)); do
  ...
  env PORT="$port" DEVICE="$dev" FAULT="$FAULT" PIPELINE="$PIPELINE" LOOP="$LOOP" \
      ...
      bash examples/run_attn_zimage_worker.sh > "${LOGDIR}/worker_${i}.log" 2>&1 &
  pid=$!
  pids+=("$pid")
...
if [ "$DETACH" = "1" ]; then
  ...
  echo "[workers]            or:   pkill -f 'multi_machine_attn_zimage.py --role worker'"
  exit 0
else
